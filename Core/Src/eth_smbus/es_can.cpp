...
typedef struct 
{
    ...
    std::array<std::pair<CAN_TxHeaderTypeDef, uint8_t[8]>, ES_CAN_BUFF_MAX_COUNT> buff;

    std::array<std::pair<bool, smbus::can::CanSocket*>, ES_APP_MAX_COUNT> callback;
} ESCan_port_t;
...
void ESCan_init(void)
{
    ...
    for(size_t port_i = 0; port_i < 2; port_i++)
    {
        ...
        for(size_t i = 0; i < ES_CAN_BUFF_MAX_COUNT; i++)
        {
...
#ifdef ES_CAN_ENABLE_RX1_BIND
    ESCan_filterSet(ESPort_1);
#endif /*ES_CAN_ENABLE_RX2_BIND*/
#ifdef ES_CAN_ENABLE_RX2_BIND
    ESCan_filterSet(ESPort_2);
#endif /*ES_CAN_ENABLE_RX1_BIND*/
}
...
void ESCan_process(void)
{
    for(size_t port_i = 0; port_i < 2; port_i++)
    {
        ...
        {
            uint32_t TxMailbox;
            auto packet = g_obj[port_i].buff[g_obj[port_i].rp & ES_CAN_BUFF_MAX_COUNT];
            HAL_StatusTypeDef result = HAL_CAN_AddTxMessage(g_obj[port_i].hcan, &packet.first, packet.second, &TxMailbox);
            if(result != HAL_OK)
            {
                ES_ERR((ESPort_t)port_i, "Can resend failed by " + std::to_string(result));
                break;
            }
            g_obj[port_i].rp++;
            g_obj[port_i].count--;
    ...
    {
        ES_ERR((ESPort_t)0, "Can getErr 0d" + std::to_string(HAL_CAN_GetError(&hcan1)));
    ...
    {
        ES_ERR((ESPort_t)1, "Can getErr 0d" + std::to_string(HAL_CAN_GetError(&hcan2)));
        HAL_CAN_ResetError(&hcan2);
        ESLed_bus_err();
    }
}

size_t smbus::can::can_bind(ESPort_t port, void* socket)
{
    size_t array_index = ES_APP_MAX_COUNT - 1;
    bool is_hit = false;
...
void smbus::can::can_send(ESPort_t port, uint16_t can_id, const uint8_t* data, size_t len)
{
    ...
    if(HAL_CAN_GetTxMailboxesFreeLevel(g_obj[port].hcan) != 0)
    {
        ...
        if(result != HAL_OK)
        {
            ES_ERR(port, "Can send failed by " + std::to_string(result));
            ES_CAN_SEND_FAIL(port);
        }
    }else{
        if(g_obj[port].count == ES_CAN_BUFF_MAX_COUNT)
        {
            g_obj[port].rp++;
            g_obj[port].count--;
            ES_CAN_OVERFLOW(port);
        }

        g_obj[port].buff[g_obj[port].wp % ES_CAN_BUFF_MAX_COUNT].first = header;
        memcpy(g_obj[port].buff[g_obj[port].wp % ES_CAN_BUFF_MAX_COUNT].second, data, len);
        g_obj[port].wp++;
        g_obj[port].count++;
    }
}

void smbus::can::set_enable(ESPort_t port, size_t array_index, bool is_enable)
{
    if(array_index < ES_APP_MAX_COUNT)
    {
